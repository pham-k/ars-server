...
func NewSlogLogger() any {
	handler := NewHandler(&slog.HandlerOptions{
		Level:       slog.LevelInfo,
		AddSource:   false,
		ReplaceAttr: nil,
	})
	pLog := slog.New(handler)
	return pLog
}

func colorizer(colorCode int, v string) string {
	return fmt.Sprintf("\033[%sm%-2s%s", strconv.Itoa(colorCode), v, reset)
	//return fmt.Sprintf("\033[%sm%-6s%s", strconv.Itoa(colorCode), v, reset)
}
...
func (h *Handler) computeAttrs(
	...
	if err := h.h.Handle(ctx, r); err != nil {
		return nil, fmt.Errorf("error when calling inner api's Handle: %w", err)
	}

	var attrs map[string]any
	err := json.Unmarshal(h.b.Bytes(), &attrs)
	if err != nil {
		return nil, fmt.Errorf("error when unmarshaling inner api's Handle result: %w", err)
	}
	return attrs, nil
}
...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	if !levelAttr.Equal(slog.Attr{}) {
		level = levelAttr.Value.String()[:1]
